    ...
    <main style={{ minHeight: "100dvh", maxWidth: 430, margin: "0 auto", background: "#0a0a0a", fontFamily: "system-ui", display: "flex", flexDirection: "column", color: "#fff" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {(["name","age","location","occupation"] as const).map((key) => (
          <div key={key}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.35)", letterSpacing: "0.08em", textTransform: "uppercase", display: "block", marginBottom: 8 }}>
              {key.charAt(0).toUpperCase() + key.slice(1)}
            </label>
            <input
              value={draft[key] ?? ""}
              onChange={e => setDraft(p => ({ ...p!, [key]: e.target.value }))}
              style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 12, padding: "11px 14px", color: "#fff", fontSize: 14, outline: "none", fontFamily: "system-ui" }}
            />
          </div>
        ))}
        ...
        <div>
          ...
          <textarea
            ...
            style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: "11px 14px", color: "#fff", fontSize: 14, outline: "none", resize: "none", lineHeight: 1.6, fontFamily: "system-ui" }}
          />
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 12, padding: "10px 14px", color: "#fff", fontSize: 13, outline: "none", fontFamily: "system-ui" }}
            />
            <button onClick={() => {
              if (newInterest.trim()) {
                setDraft(p => ({ ...p!, interests: [...(p!.interests as string[] ?? []), newInterest.trim()] }));
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
            {INTEREST_SUGGESTIONS.filter(s => !(draft.interests as string[] ?? []).includes(s)).map((s: string) => (
              <div key={s} onClick={() => setDraft(p => ({ ...p!, interests: [...(p!.interests as string[] ?? []), s] }))} style={{ display: "inline-flex", alignItems: "center", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.5)", borderRadius: 50, padding: "4px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>+ {s}</div>
            ))}
          </div>
        </div>
