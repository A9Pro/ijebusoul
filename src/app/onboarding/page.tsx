...
// ─── Shell lives OUTSIDE the page component so it never remounts on re-render ───
...
const Shell = ({ children, progress, onBack }: ShellProps) => (
  <main style={{
    ...
    padding: "0 24px 52px", maxWidth: 430, margin: "0 auto", overflow: "hidden",
  }}>
    {/* Background image */}
    <div style={{
      ...
      backgroundColor: "#1a0a00", // fallback if image missing
      zIndex: 0,
    }} />
    {/* Overlay */}
    <div style={{
      position: "absolute", inset: 0,
      background: "linear-gradient(to bottom, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.88) 65%, rgba(0,0,0,0.97) 100%)",
      zIndex: 1,
    }} />
    {/* Progress bar */}
...
if (step === 0) {
  ...
  const errorBox = error ? (
    <div style={{
      ...
      borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#FF3366", marginBottom: 20,
    }}>
      {error}
    </div>
  ) : null;
    ...
    <Shell progress={progress} onBack={back}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          ...
          <input
            type="text" placeholder="e.g. Adunola"
        ...
        <div>
          ...
          <input
            type="number" placeholder="e.g. 24" min="18" max="100"
    ...
    <Shell progress={progress} onBack={back}>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {PREFERENCES.map(p => (
            <button key={p} onClick={() => update("preference", p)} style={{
          ...
          <div key={group.lga}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {group.towns.map(town => (
                <button key={town} onClick={() => { update("location", town); setTimeout(() => setStep(s => s + 1), 300); }} style={{
  ...
  if (step === 4) return (
    <Shell progress={progress} onBack={back}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {LOOKING_FOR.map(opt => (
          <button key={opt.id} onClick={() => update("lookingFor", opt.id)} style={{
    ...
    <Shell progress={progress} onBack={back}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} onClick={() => !photos[i] && fileRef.current?.click()} style={{
            ...
            overflow: "hidden", cursor: photos[i] ? "default" : "pointer",
