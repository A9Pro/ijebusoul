...
const inputStyle: React.CSSProperties = {
  ...
  padding: "16px 18px", fontSize: 16, color: "#fff", outline: "none", fontFamily: "system-ui",
};
...
export default function OnboardingPage() {
    ...
    if (step === totalSteps - 1) {
      ...
      try {
        ...
        for (const p of photos) {
          const path = await uploadAvatar(p.file, userId);
        ...
        const { error: err } = await supabase.from("profiles").insert({
          ...
          avatar_url:  uploadedPaths[0] ?? null,
          interests:   [],
        });
        if (err) throw err;
        router.push("/home");
  ...
  const Shell = ({ children }: { children: React.ReactNode }) => (
    <main style={{ minHeight: "100dvh", width: "100%", position: "relative", fontFamily: "system-ui", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", padding: "0 24px 52px", maxWidth: 430, margin: "0 auto", overflow: "hidden" }}>
      ...
      <button onClick={back} style={{ position: "absolute", top: 44, left: 24, zIndex: 4, background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 50, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, color: "#fff" }}>←</button>
  ...
  const NextBtn = ({ label = "Continue →", disabled = false }: { label?: string; disabled?: boolean }) => (
    <button onClick={next} disabled={disabled || loading} style={{ width: "100%", background: disabled || loading ? "rgba(212,175,55,0.3)" : "#D4AF37", color: disabled || loading ? "rgba(0,0,0,0.35)" : "#000", fontSize: 16, fontWeight: 800, border: "none", padding: "18px 0", borderRadius: 14, cursor: disabled || loading ? "not-allowed" : "pointer", marginTop: 24 }}>
  ...
  if (step === 0) return (
    <Shell>
      ...
      <NextBtn disabled={!canNext()} label="Create account →" />
    </Shell>
  );
  ...
  if (step === 5) return (
    <Shell>
      ...
      {errorBox}
      <NextBtn disabled={!canNext()} label={photos.length > 0 ? "Continue →" : "Add at least 1 photo"} />
    </Shell>
  );
  ...
  return (
    <Shell>
      ...
      {errorBox}
      <textarea placeholder="e.g. Ijebu girl who loves suya nights and deep convos. Looking for someone who matches my energy..." value={form.bio} onChange={e => update("bio", e.target.value)} maxLength={300} rows={6} style={{ ...inputStyle, resize: "none", lineHeight: 1.6, paddingTop: 16 }} />
